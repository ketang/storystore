...
DEFAULT_SURFACE_KINDS: frozenset[str] = frozenset({"cli-command", "http-route", "bin"})
...
def _score_evidence(story: Any) -> int:
    ...
    if section_text and _has_placeholder_marker(section_text):
        return 0
    total = len(tests) + len(surface) + len(docs)
    subsections = sum(1 for group in (tests, surface, docs) if group)
    if total == 0:
        return 0
